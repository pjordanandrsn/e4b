"""experts-4bit — install/import alias for the ``experts4bit-qlora`` distribution.

``pip install experts-4bit`` installs the real package (``experts4bit-qlora``) and
this shim re-exports it, so ``import experts_4bit`` is identical to
``import experts4bit_qlora`` — submodules included, and
``experts_4bit is experts4bit_qlora`` is True.

The canonical package is ``experts4bit_qlora``; this alias exists only so the
name "experts-4bit" resolves on PyPI and in ``import`` instead of dead-ending.
