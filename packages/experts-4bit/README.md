...
```bash
pip install experts-4bit
```
```python
import experts_4bit   # identical to `import experts4bit_qlora`
```

`experts-4bit` is one of several short/variant names for **experts4bit-qlora** —
...
Extras forward: `pip install experts-4bit[train]` / `[serve]`. MIT © Cerin Amroth.
